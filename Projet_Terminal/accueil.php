...
?>
    ...
    echo '<option value="'.$ligne['NomUtil'].'"></option>';  //on créer une option par utilisateur pour la barre de recherche. On les classent par ordre d'abonées.
    }
     ?>
</datalist>
<!-- Barre de recherche  -->
...
<button type="submit" class="bouton">rechercher</button>
    ...
    //Dans le cas où on a chercher avec la barre de recherche un utilisateur alors on va aller sur son profil
        if(isset($_POST["recherche"])){
            $userChercher= $_POST["recherche"];
            header ("Location:profil.php?user=$userChercher");
        }
    //Dans le cas où on n'est pas connecté on va juste montrer les commentaires les plus liker.
    }else{
